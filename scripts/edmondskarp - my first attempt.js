...
function EdmondsKarp(vertex_set, edge_set, cb) {
   ...
   function findAugmentingPath(residual_graph) {
     ...
     // Performs the breadth-first search to check if there is flow from s --> t
     // Also calculate the levels of each node
     function bfs(graph) {
       var parents = [];    // keep track of a node's parent
       var level = [];    // keep track of node level (also whether visited)
       ...
       for (var j; j < (graph.nodes()).length; j+=1) {
           level.push(-1)
       }
...
// and subtracting that value from the backward edges.
function augment(path, residual_graph, levels) {
    function bottleneck(path, residual_graph, levels) {
      ...
      for (var v; v = path.pop();) {
        ...
        if ((edge.flow < min) && (levels[v] === (levels[u]+1))) {min = edge.flow;}
    ...
    var b = bottleneck(path.slice(0), residual_graph, levels);
    var u = 0;
